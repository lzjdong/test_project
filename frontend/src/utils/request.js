...
service.interceptors.request.use(
  (config) => {
    ...
    if (auth) {
      const { realm, clientId, token } = auth;
      config.headers = realm ? { ...config.headers, Realm: realm } : config.headers;
      config.headers = clientId ? { ...config.headers, 'Client-Name': clientId } : config.headers;
...
//   if (
//     response.headers[LinkTokenKey.toLocaleLowerCase()] ||
//     (response.config.headers && response.config.headers[LinkTokenKey.toLocaleLowerCase()])
//   ) {
//     const linkToken =
//       response.headers[LinkTokenKey.toLocaleLowerCase()] ||
//       response.config.headers[LinkTokenKey.toLocaleLowerCase()];
